...
class SharedEncoder_Nolayer4(torch_nn.Module):
    """
    孪生编码器:流 A 和 流 B 共享这个网络。
    使用 ResNet18/34/50 作为 backbone 提取染色体的深度语义特征。
    """
...
class CrossAttentionAlignment_cross(torch_nn.Module):
    """
    交叉注意力对齐模块：用于克服染色体自由弯曲带来的空间不对齐。
    思想：以健康的染色体 A 为基准 (Query)，去异常染色体 B 中寻找相似的结构 (Key, Value) 并拼凑对齐。
    """
    def __init__(self, channels):
        super(CrossAttentionAlignment_cross, self).__init__()
...
class CrossAttentionAlignment_MHA_cross(torch_nn.Module):
    """
    ...
    def __init__(self, channels, num_heads=8):
        ...
        """
        super(CrossAttentionAlignment_MHA_cross, self).__init__()
...
class HomBlock_W_diff(torch_nn.Module):
    def __init__(self, channels, num_heads=8):
        super(HomBlock_W_diff, self).__init__()
...
class HomBlock_abs(torch_nn.Module):
    #差值
    def __init__(self, channels, num_heads=8):
        super(HomBlock_abs, self).__init__()
        self.multihead_attn = torch_nn.MultiheadAttention(
            embed_dim=channels, 
            num_heads=num_heads, 
            batch_first=True
...
class HomBlock_Pre_LN(torch_nn.Module):
    # 差值特征:|Q - Align(Q,K,V)|；Pre-LN + 注意力 dropout 提升稳定性与泛化
    def __init__(self, channels, num_heads=8, attn_dropout=0.1):
        super(HomBlock_Pre_LN, self).__init__()
        self.token_norm = torch_nn.LayerNorm(channels)
        self.multihead_attn = torch_nn.MultiheadAttention(
            embed_dim=channels,
            num_heads=num_heads,
...
class SiameseAnomalyNet(torch_nn.Module):
    ...
    def __init__(self, in_channels=3, backbone='resnet18', pretrained=False, 
        ...
        # 2. 对齐模块
        self.aligner = HomBlock_abs(channels=feat_channels, num_heads=8)
...
class SiameseAnomalyNet_conv1_gap_gmp(torch_nn.Module):
    """
    完整的双流异常检测网络 (弱监督定位版) - ResNet Backbone
    """
    def __init__(self, in_channels=3, backbone='resnet18', pretrained=False, 
